...
def view_your_snakes():
    ...
    for s in snakes:
        print(' * {} is a {} that is {} meters in length, and is {}venomous.'.format(
            s.name,
            s.species,
            s.length,
...
def book_a_cage():
    ...
    if checkin >= checkout:
        hosts.error_msg('Check-in must be before check-out date.')
        return
